 ...
 */
//区域
import Area from "Qbase.areas.js";
...
Qbase.StringUtils = {
	 ...
	 * @param {String} datas
	*/
	formatJson: function(datas) {
		let reg = /("([^\\\"]*(\\.)?)*")|('([^\\\']*(\\.)?)*')|(\/{2,}.*?(\r|\n))|(\/\*(\n|.)*?\*\/)/g;// 正则表达式
		return datas.replace(reg, function(word) { // 去除注释后的文本
	...
	isNest: function(rule, str) {
		...
		var stack = [];
		for(var i = 0; i < len; i++) {
			if(Qbase.ArrayUtils.inArray(keys, chs[i])) {
				stack.push(rule[chs[i]]);
			} else {
				if(chs[i] === stack[stack.length - 1]) {
					stack.pop();
				} else if(Qbase.ArrayUtils.inArray(values, chs[i])) {
					return false;
				}
			}
		}
...
Qbase.IdCardUtils = {
	...
	getPersonInfo18: function(idCard) {
		...
		var address = '';
		var sex = '';
		address = Qbase.areas[idCard.substr(0, 2) + '0000'] + ' ' + Qbase.areas[idCard.substr(0, 4) + '00'] + ' ' + Qbase.areas[idCard.substr(0, 6)];
...
Qbase.DateUtils = {
	...
	formt: function(dateTime, pattern) {
		...
		if(Qbase.StringUtils.isBlank(pattern)) {
			return date.toLocaleString();
		}
		return pattern.replace(/([a-z])\1*/ig, function(matchStr, group1) {
			var replacement = "";
			switch(group1) {
				case Qbase.DateUtils.patterns.PATTERN_ERA: //G
					break;
				case Qbase.DateUtils.patterns.PATTERN_WEEK_YEAR: //Y
				case Qbase.DateUtils.patterns.PATTERN_YEAR: //y
					replacement = date.getFullYear();
					break;
				case Qbase.DateUtils.patterns.PATTERN_MONTH: //M
					var month = date.getMonth() + 1;
					replacement = (month < 10 && matchStr.length >= 2) ? "0" + month : month;
					break;
				case Qbase.DateUtils.patterns.PATTERN_DAY_OF_MONTH: //d
					var days = date.getDate();
					replacement = (days < 10 && matchStr.length >= 2) ? "0" + days : days;
					break;
				case Qbase.DateUtils.patterns.PATTERN_HOUR_OF_DAY1: //k(1~24)
					var hours24 = date.getHours();
					replacement = hours24;
					break;
				case Qbase.DateUtils.patterns.PATTERN_HOUR_OF_DAY0: //H(0~23)
					var hours24 = date.getHours();
					replacement = (hours24 < 10 && matchStr.length >= 2) ? "0" + hours24 : hours24;
					break;
				case Qbase.DateUtils.patterns.PATTERN_MINUTE: //m
					var minutes = date.getMinutes();
					replacement = (minutes < 10 && matchStr.length >= 2) ? "0" + minutes : minutes;
					break;
				case Qbase.DateUtils.patterns.PATTERN_SECOND: //s
					var seconds = date.getSeconds();
					replacement = (seconds < 10 && matchStr.length >= 2) ? "0" + seconds : seconds;
					break;
				case Qbase.DateUtils.patterns.PATTERN_MILLISECOND: //S
					var milliSeconds = date.getMilliseconds();
					replacement = milliSeconds;
					break;
				case Qbase.DateUtils.patterns.PATTERN_DAY_OF_WEEK: //E
					var day = date.getDay();
					replacement = Qbase.DateUtils.week['ch'][day];
					break;
				case Qbase.DateUtils.patterns.PATTERN_DAY_OF_YEAR: //D
					replacement = Qbase.DateUtils.dayOfTheYear(date);
					break;
				case Qbase.DateUtils.patterns.PATTERN_DAY_OF_WEEK_IN_MONTH: //F
					var days = date.getDate();
					replacement = Math.floor(days / 7);
					break;
				case Qbase.DateUtils.patterns.PATTERN_WEEK_OF_YEAR: //w
					var days = Qbase.DateUtils.dayOfTheYear(date);
					replacement = Math.ceil(days / 7);
					break;
				case Qbase.DateUtils.patterns.PATTERN_WEEK_OF_MONTH: //W
					var days = date.getDate();
					replacement = Math.ceil(days / 7);
					break;
				case Qbase.DateUtils.patterns.PATTERN_AM_PM: //a
					var hours24 = date.getHours();
					replacement = hours24 < 12 ? "\u4e0a\u5348" : "\u4e0b\u5348";
					break;
				case Qbase.DateUtils.patterns.PATTERN_HOUR1: //h(1~12)
					var hours12 = date.getHours() % 12 || 12; //0转为12
					replacement = (hours12 < 10 && matchStr.length >= 2) ? "0" + hours12 : hours12;
					break;
				case Qbase.DateUtils.patterns.PATTERN_HOUR0: //K(0~11)
					var hours12 = date.getHours() % 12;
					replacement = hours12;
					break;
				case Qbase.DateUtils.patterns.PATTERN_ZONE_NAME: //z
					replacement = Qbase.DateUtils.getZoneNameValue(date)['name'];
					break;
				case Qbase.DateUtils.patterns.PATTERN_ZONE_VALUE: //Z
					replacement = Qbase.DateUtils.getZoneNameValue(date)['value'];
					break;
				case Qbase.DateUtils.patterns.PATTERN_ISO_DAY_OF_WEEK: //u
					break;
				case Qbase.DateUtils.patterns.PATTERN_ISO_ZONE: //X
					break;
				default:
					break;
			}
...
//简单四则运算
Qbase.ElementaryArithmeticUtils = {
	//运算符优先级
	operatorPrecedence: {
		'+': 0,
		'-': 0,
	...
	infixToPostfixExpression: function(expression) {
		expression = Qbase.StringUtils.deleteWhitespace(expression);
		...
		for(var i = 0; i < size; i++) {
			if(Qbase.StringUtils.isNumeric(elementArr[i])) {
				//如果是数值
	...
	calculateByPrefixExpression: function(expression) {
		...
		for(var i = size - 1; i >= 0; i--) {
			if(Qbase.StringUtils.isNumeric(elementArr[i])) {
				...
				switch(elementArr[i]) {
					case '+':
						result = Qbase.ElementaryArithmeticUtils.add(operand1, operand2);
						break;
					case '-':
						result = Qbase.ElementaryArithmeticUtils.subtract(operand1, operand2);
						break;
					case '×':
					case '*':
						result = Qbase.ElementaryArithmeticUtils.multiply(operand1, operand2);
						break;
					case '\/':
					case '÷':
						result = Qbase.ElementaryArithmeticUtils.divide(operand1, operand2);
						break;
					default:
						result = '';
	...
	calculateByPostfixExpression: function(expression) {
		...
		for(var i = 0; i < size; i++) {
			if(Qbase.StringUtils.isNumeric(elementArr[i])) {
				...
				switch(elementArr[i]) {
					case '+':
						result = Qbase.ElementaryArithmeticUtils.add(operand2, operand1);
						break;
					case '-':
						result = Qbase.ElementaryArithmeticUtils.subtract(operand2, operand1);
						break;
					case '×':
					case '*':
						result = Qbase.ElementaryArithmeticUtils.multiply(operand2, operand1);
						break;
					case '\/':
					case '÷':
						result = Qbase.ElementaryArithmeticUtils.divide(operand2, operand1);
						break;
					default:
						result = '';
						alert("The operator 【" + elementArr[i] + "】 is not legal");
...
Qbase.RegexUtils = {
	...
	createRegexObjMustExclude: function(input, conditions) {
		...
		var subPattern = "";
		for(var i = 0; i < size; i++) {
			targetStrArr[i] = Qbase.StringUtils.escapeMetacharacterOfStr(targetStrArr[i]);
			subPattern += targetStrArr[i];
			if(i != size - 1) {
				subPattern += "|";
			}
		}
		regex = Qbase.StringUtils.format(regex, [subPattern]);
		switch(matcherFlag) {
			case '0':
				regex += "\\d";
				break;
		...
		regex += Qbase.StringUtils.isNotBlank(length) ? "{" + length + "}" : "+";
		regex += "$";
